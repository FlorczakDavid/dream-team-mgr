...
export default {
    ...
    methods: {
        async getAllLangTechs() {
            try {
            const response = await axios.get("http://localhost:8080/langTechs");
        ...
        async fetchLangTech() {// fetch top 5 alphabetical on each input
            ...
            try {
            const response = await axios.get("http://localhost:8080/langTechs/"+this.inputStack);
...
<template>
    <label for="stackName" class="fs-5 form-label mb-2 mt-4">{{ $t('formProject.languagesAndTechnologies') }}</label>
    <div>
